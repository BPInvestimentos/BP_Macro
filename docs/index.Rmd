...
---
background-image: url("bp_capa.png")
background-size: contain

---
layout:true
background-image: url("BP_BP_Hor_PB.png")
...
# segunda-feira
dt_inicio_semana = as.Date("2022-01-14")

# sexta-feira
dt_fim_semana = as.Date("2022-01-21")
dt_commoditie_final_semana = "21/01/2022"
...
LimpaBase <- function(Tabela){
  ...
  Tabela %>%
    dplyr::mutate(Semana = epiweek(date),
                  Ano = year(date)) %>%
...
PlotaRetornoSemanal <- function(Tabela, Nome, Fonte, step) {
  ...
  Tabela <- Tabela %>%
    ...
    geom_text(aes(label = paste0(format(round(VarSemanal*100,2), big.mark = ".", decimal.mark = ","),"%"),
                  vjust = ifelse(VarSemanal >= 0, -0.5, 1.5)),size = 2.2,
 ...
 #   scale_y_continuous(labels = scales::percent_format(big.mark = ".", decimal.mark = ",")) + 
    scale_y_continuous(labels = scales::percent_format(big.mark = ".", decimal.mark = ","),
                       breaks = seq(-1,1,step),
                       limits = c(min(Tabela$VarSemanal)-0.1, max(Tabela$VarSemanal)+0.1)) +
...
#    filter(dia %in% c("sexta-feira")) %>% 
  ...
  graph_ewz <- ggplot(ewz_fil_var, aes(x = semana,  y = var_semanal, fill = Color)) + 
    ...
    geom_text(aes(label = paste0(format(round(var_semanal*100,2), big.mark = ".", decimal.mark = ","),"%"),
                  vjust = ifelse(var_semanal >= 0, -0.5, 1.5)),size = 2.2,
              color = 'black',fontface='bold') +
   scale_fill_manual(values = c("#27ae60","#c0392b")) +
 #      scale_fill_manual(values = c("#c0392b", "#27ae60")) +
    geom_hline(yintercept = 0) +
    theme_minimal() +
     scale_y_continuous(labels = scales::percent_format(big.mark = ".", decimal.mark = ","),
                       breaks = seq(-1,1,0.05),
                       limits = c(min(ewz_fil_var$var_semanal)-0.1,max(ewz_fil_var$var_semanal)+ 0.1)) + 
     theme(legend.position = "none",
          strip.text = element_text(face = "bold", color = "#2E2C2F", size = "11"),
          plot.title = element_text(size=15, hjust = 0.5, face="bold"),
          axis.title.x = element_text(face="bold", color="#000000",size = 9),
...
var_semanal_bovespa <- function() {
  
  
  # Obtain Treasury yield data
  bvspa <- getSymbols(Symbols = "^BVSP", src = "yahoo", auto.assign = FALSE)
  
  
  # coluna com data
  bvspa <- data.frame(date=index(bvspa), coredata(bvspa))
  
  # filtrando a partir de 2020
  bvspa_fil <- bvspa %>%
    dplyr::select(date, BVSP.Close) %>% 
    dplyr::rename(Close = "BVSP.Close") %>% 
    dplyr::filter(date >= "2022-01-01")
  
  # removendo NA
  bvspa_fil <- na.omit(bvspa_fil)
  
  bvspa_fil_var <- bvspa_fil %>%
  #  filter(date >= "2021-01-01") %>%
    as.data.frame() %>% 
    mutate(dia = weekdays(date),
           semana = epiweek(date)) %>% 
...
#    filter(dia %in% c("sexta-feira")) %>% 
  ...
  DataMaisRecente <- bvspa_fil_var %>% 
    slice(which.max(date)) %>% 
    select(date)
  
  
  
  graph_bvsap <- ggplot(bvspa_fil_var, aes(x = semana,  y = var_semanal, fill = Color)) + 
    geom_col() + 
    geom_text(aes(label = paste0(format(round(var_semanal*100,2), big.mark = ".", decimal.mark = ","),"%"),
                  vjust = ifelse(var_semanal >= 0, -0.5, 1.5)),size = 2.2,
              color = 'black',fontface='bold') +
 ...
 #      scale_fill_manual(values = c("#c0392b", "#27ae60")) +
    ...
    theme_minimal() +
     scale_y_continuous(labels = scales::percent_format(big.mark = ".", decimal.mark = ","),
                       breaks = seq(-1,1,0.05),
                       limits = c(min(bvspa_fil_var$var_semanal)-0.1,max(bvspa_fil_var$var_semanal)+ 0.1)) + 
     theme(legend.position = "none",
          strip.text = element_text(face = "bold", color = "#2E2C2F", size = "11"),
          plot.title = element_text(size=15, hjust = 0.5, face="bold"),
          axis.title.x = element_text(face="bold", color="#000000",size = 9),
          ...
          axis.text.y = element_text(face="bold", color="#000000",size = 8),
          axis.text.x = element_text(face="bold", color="#000000",size = 8)) +
    labs(title = paste("Ibovespa - Variação Semanal"),
         y = "Variação",
         x = "Semana",
         subtitle = paste("Última atualização:", 
                          format(DataMaisRecente, "%d/%m/%Y")),
...
var_semanal_dolar <- function() {
  
  
  # Obtain Treasury yield data
  dolar <- getSymbols(Symbols = "USDBRL=X", src = "yahoo", auto.assign = FALSE)
  
  
  # coluna com data
  dolar <- data.frame(date=index(dolar), coredata(dolar))
  
  # filtrando a partir de 2020
  dolar_fil <- dolar %>%
    dplyr::select(date, USDBRL.X.Close) %>% 
    dplyr::rename(Close = "USDBRL.X.Close") %>% 
    dplyr::filter(date >= "2022-01-01")
  
  # removendo NA
  dolar_fil <- na.omit(dolar_fil)
  
  dolar_fil_var <- dolar_fil %>%
  #  filter(date >= "2021-01-01") %>%
    as.data.frame() %>% 
    mutate(dia = weekdays(date),
           semana = epiweek(date)) %>% 
...
#    filter(dia %in% c("sexta-feira")) %>% 
  ...
  DataMaisRecente <- dolar_fil_var %>% 
    slice(which.max(date)) %>% 
    select(date)
  
  
  
  graph_dolar <- ggplot(dolar_fil_var, aes(x = semana,  y = var_semanal, fill = Color)) + 
    geom_col() + 
    geom_text(aes(label = paste0(format(round(var_semanal*100,2), big.mark = ".", decimal.mark = ","),"%"),
                  vjust = ifelse(var_semanal >= 0, -0.5, 1.5)),size = 2.2,
              color = 'black',fontface='bold') +
 ...
 #      scale_fill_manual(values = c("#c0392b", "#27ae60")) +
    ...
    theme_minimal() +
     scale_y_continuous(labels = scales::percent_format(big.mark = ".", decimal.mark = ","),
                       breaks = seq(-1,1,0.05),
                       limits = c(min(dolar_fil_var$var_semanal)-0.1,max(dolar_fil_var$var_semanal)+ 0.1)) + 
     theme(legend.position = "none",
          strip.text = element_text(face = "bold", color = "#2E2C2F", size = "11"),
          plot.title = element_text(size=15, hjust = 0.5, face="bold"),
          axis.title.x = element_text(face="bold", color="#000000",size = 9),
          ...
          axis.text.y = element_text(face="bold", color="#000000",size = 8),
          axis.text.x = element_text(face="bold", color="#000000",size = 8)) +
    labs(title = paste("Dólar - Variação Semanal"),
         y = "Variação",
         x = "Semana",
         subtitle = paste("Última atualização:", 
                          format(DataMaisRecente, "%d/%m/%Y")),
...
# 
# 
# write.csv2(indicadores_setores_v2, "indicadores_setores_v2.csv", row.names = F)
...
indicadores_setores_v2 <- read.csv2("indicadores_setores_v2.csv")
...
acoes_setores_v3 <- acoes_setores_v2 %>%
  mutate(dia = weekdays(date)) %>% 
  dplyr::filter(volume >= 500000) %>% 
  dplyr::filter(!symbol %in% tickets_repetidos) %>% 
  dplyr::filter(dia %in% c("terça-feira", "quinta-feira")) %>% 
  dplyr::group_by(symbol) %>% 
  dplyr::mutate(var_semanal =  (close/lag(close))-1) %>% 
  dplyr::filter(dia %in% c("quinta-feira")) %>% 
  dplyr::mutate(Color = ifelse(var_semanal < 0, "negativo","green")) %>% 
  dplyr::filter(date == max(date)) %>% 
  dplyr::mutate(nome_acao_setor = paste0(Empresa, " (",Ações, ")")) %>% 
  dplyr::select(nome_acao_setor, date, var_semanal, dia, Color, Setor)
...
acoes_setores_v3_maiores <- acoes_setores_v3 %>% 
  ...
  filter(Setor %in% c("Bens Industriais", "Consumo Cíclico",
                      ...
                      "Tecnologia da Informação", "Utilidade Pública", "Saúde"))
...
brent_graph <- ggplot(brent_futuro, aes(x = data, y = Close)) +
  ...
  labs(y="Preço do contrato futuro (em Dólar)", x="", fill="",
       title = "Petróleo Brent Futuros - Jan 22 (BZ1)",
...
soja_graph <- ggplot(soja_futuro, aes(x = data, y = Close)) +
  geom_line(size = 1.1, colour = "#ffa600") +
  labs(y="Preço do contrato futuro (em Dólar)", x="", fill="",
       title = "Soja NY Futuros - Jan 22 (ZSX1)",
       caption = "Fonte: Investing.com | Elaboração: BP Investimentos",
       colour = "") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
milho_graph <- ggplot(milho_futuro, aes(x = data, y = Close)) +
  geom_line(size = 1.1, colour = "#ffa600") +
  labs(y="Preço do contrato futuro (em Dólar)", x="", fill="",
       title = "Milho NY Futuros - Jan 22 (ZCZ1)",
       caption = "Fonte: Investing.com | Elaboração: BP Investimentos",
       colour = "") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
algodao_graph <- ggplot(algodao_futuros, aes(x = data, y = Close)) +
  geom_line(size = 1.1, colour = "#ffa600") +
  labs(y="Preço do contrato futuro (em Dólar)", x="", fill="",
       title = "Algodão NY nº2 Futuros - Jan 22 (CTZ1)",
       caption = "Fonte: Investing.com | Elaboração: BP Investimentos",
       colour = "") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
boi_graph <- ggplot(boi_gordo_futuros, aes(x = data, y = Close)) +
  ...
  labs(y="Preço do contrato futuro (em Real)", x="", fill="",
       title = "Boi Gordo Futuros - Jan 22 (BGIc1)",
       caption = "Fonte: Investing.com | Elaboração: BP Investimentos",
       colour = "") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
GetTDData::download.TD.data('NTN-B')
ntnb <- GetTDData::read.TD.files(dl.folder = 'TD Files',
                                 asset.codes = 'NTN-B')
...
# 
#  #cdi <- cdi %>% filter(date <= "2021-11-26")
#  cdi[3030,1] <- as.Date("2022-01-21")
#  cdi[3030,2] <- as.numeric(9.15)
# 
# ####################################################################
...
curva_seg <- read.csv2("curva-pre_2022-01-14.csv")

curva_sex <- read.csv2("curva-pre_2022-01-21.csv")
...
inf_mensal_graph <- ggplot(data=inflacao_12, aes(x = date, y = inflacao_mensal, fill = Color)) +
  ...
  geom_text(aes(label = paste0(format(round(inflacao_mensal,2), big.mark = ".", decimal.mark = ","),"%"),
                vjust = ifelse(inflacao_mensal >= 0, -0.5, 1.5)),size = 3,
            color = 'black',fontface='bold') +
  theme_minimal() +
  theme(legend.position = "none",
          strip.text = element_text(face = "bold", color = "#2E2C2F", size = "11"),
          ...
          axis.text.x = element_text(face="bold", color="#000000",size = 7)) +
...
inflacao_grupo <- inflacao_grupo %>% filter("2021-01-01" <= date & date <= "2021-12-01")
...
vertical_sep_ipca <- ggplot(contribuicao2, aes(x=variação,y=setor,fill=Color))+
  geom_col() +
  labs(x="", y="", subtitle = "Variação acumulada em 12 meses (%): 10,06", fill="",
       caption = "Fonte: Índice de Preços ao Consumidor Amplo (IPCA) - IBGE | Elaboração: BP Investimentos",
...
---
background-image: url("var_semanal.png")
...
# 
# PlotaRetornoSemanal_ibov <- function(Tabela, Nome, Fonte, step) {
#   
#   DataMaisRecente = as.character.Date(max(Tabela$date[is.numeric(Tabela$value)]), 
#                                       format = "%d/%m/%Y")
#   
...
# ibov_graph <- PlotaRetornoSemanal(Ibov, "Ibovespa", "B3", .05)
# ibov_graph



ibov_graph <- var_semanal_bovespa()
ibov_graph
...
PlotaRetornoSemanal_sp500 <- function(Tabela, Nome, Fonte, step) {
...
sp500_plot <- PlotaRetornoSemanal_sp500(SP500, "S&P 500", "FED", .05)
sp500_plot
...
# dolar_graph <- PlotaRetornoSemanal(Dolar, "Dólar", "Banco Central", .05)
# dolar_graph


dolar_graph <- var_semanal_dolar()
dolar_graph
...
ggplot(acoes_setores_v3_maiores,
       aes(x = var_semanal,  y = reorder(nome_acao_setor, +var_semanal), fill = Color)) + 
  geom_bar(stat = "identity") + 
  geom_text(aes(label = paste0(format(round(var_semanal*100,2), big.mark = ".", decimal.mark = ","),"%"),
                hjust = ifelse(var_semanal >= 0, -0.2, 1.3)),size = 2.2,
            color = 'black',fontface='bold') +
  scale_fill_manual(values = c("#27ae60","#c0392b")) +
 # geom_hline(yintercept = 5.5, linetype = "dashed") +
  ...
  scale_x_continuous(labels = scales::percent_format(big.mark = ".", decimal.mark = ",", accuracy = 1),
                     breaks = seq(min(acoes_setores_v3_maiores$var_semanal)-0.05,
                                  max(acoes_setores_v3_maiores$var_semanal)+ 0.05,
                                  0.05),
                     limits = c(min(acoes_setores_v3_maiores$var_semanal)-0.05,
                                max(acoes_setores_v3_maiores$var_semanal)+ 0.05)) +
  labs(title = "5 ações com as melhores performances em seu setor \nna última semana",
       subtitle = "Variação semanal no valor da ação",
       x = "Variação na semana (%)",
       y = "",
       caption = paste("Fonte: Yahoo Finance! | Elaboração: BP Investimentos\nNota: Apenas ações com movimentação de ao menos R$ 500.000,00 por dia.")) +
  theme_xaringan(text_font_size = 10)
...
ggplot(acoes_setores_v3_menores,
       aes(x = var_semanal,  y = reorder(nome_acao_setor, +var_semanal), fill = Color)) + 
  geom_bar(stat = "identity") + 
  geom_text(aes(label = paste0(format(round(var_semanal*100,2), big.mark = ".", decimal.mark = ","),"%"),
 ...
 # scale_fill_manual(values = c("#c0392b", "#27ae60")) +
  ...
  facet_wrap(~Setor, scales = "free") +
  scale_x_continuous(labels = scales::percent_format(big.mark = ".", decimal.mark = ",", accuracy = 1),
                     breaks = seq(min(acoes_setores_v3_menores$var_semanal)-0.1,
                                  max(acoes_setores_v3_menores$var_semanal)+ 0.1,
                                  0.1),
                     limits = c(min(acoes_setores_v3_menores$var_semanal)-0.1,
                                max(acoes_setores_v3_menores$var_semanal)+ 0.1)) +
  labs(title = "5 ações com as piores performances em seu setor \nna última semana",
       subtitle = "Variação semanal no valor da ação",
       x = "Variação na semana (%)",
       y = "",
       caption = paste("Fonte: Yahoo Finance! | Elaboração: BP Investimentos\nNota: Apenas ações com movimentação de ao menos R$ 500.000,00 por dia.")) +
...
ggplot(dataset_indices, aes(x=date,y = ret_acum)) + 
   ...
   theme(legend.position = "none",
    ...
    scale_x_date(date_labels = "%b\n%Y", breaks = "3 month", expand = c(0,30)) +
...
```{r echo=FALSE}
 ...
 theme(legend.position = "bottom",
  ...
  scale_y_continuous(breaks = c(0.1,0.5,1,1.5,2,2.5),
                     ...
                     limits = c(0.1,2.5)) +
  scale_x_date(date_labels = "%b\n%Y", breaks = "1 month", expand = c(0,15)) +
    theme_xaringan(text_font_size = 10)
...
---
background-image: url("commodities.png")
background-position: center
background-size: contain
...
---
background-image: url("renda_fixa.png")
background-position: center
background-size: contain
...
---
background-image: url("inflacao.png")
background-position: center
background-size: contain
